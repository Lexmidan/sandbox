...
class Simulation:
    ...
    def init_particles(self, n, radius, styles=None):
        ...
        for i, rad in enumerate(radius):
            ...
            while True:
                ...
                # #vzdal=(np.sqrt((x-.5)**2+(y-.5)**2))
                # vx = (x**2+0.1)*np.sign(np.random.random()-0.5)
                # vy = (x**2+0.1)*np.sign(np.random.random()-0.5)
                
                #CENTER
                x=0.5+random.normal(loc=0.0, scale=.15)
                y=0.5+random.normal(loc=0.0, scale=.15)
                vzdal=(np.sqrt((x-.5)**2+(y-.5)**2))
                vx = 1e-3/(vzdal**2)*np.sign(np.random.random()-.5)
                vy = 1e-3/(vzdal**2)*np.sign(np.random.random()-.5)
    ...
    def do_animation(self, save=True):
        ...
        anim = animation.FuncAnimation(fig, self.animate, init_func=self.init,
                               frames=800, interval=2, blit=True) #frames=800
        if save:
            Writer = animation.writers['ffmpeg']
            writer = Writer(fps=30, bitrate=1800) #fps=100, bitrate=1800
            anim.save('C:/Users/aleks/Projects/arameic-mishmash/Bachelor/collision.mp4', writer=writer)
        else:
            plt.show()
